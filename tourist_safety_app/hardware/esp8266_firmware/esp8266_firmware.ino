...
#define NETWORK_SSID       "vivo Y16"
#define NETWORK_PASS       "12341234"

#define FIREBASE_AUTH_KEY  "AIzaSyBT7-1zwvoHGOwfjHTX-D3Xt2p8BzBphso"
#define FIREBASE_DB_URL    "https://smarttouristapp-8041d-default-rtdb.asia-southeast1.firebasedatabase.app"
...
void fetchNodeRegistration() {
  ...
  // Read userId directly for maximum reliability on ESP8266
  if (Firebase.RTDB.getString(&fbDataObj, devicePath + "/userId")) {
    String fetchedUid = fbDataObj.stringValue();

    if (fetchedUid.length() > 3 && fetchedUid != "null") {
      activeSession.uid = fetchedUid;
      activeSession.isPaired = true;

      if (Firebase.RTDB.getString(&fbDataObj, devicePath + "/userName")) {
        activeSession.displayName = fbDataObj.stringValue();
      } else {
        activeSession.displayName = "Tourist";
      }

      if (Firebase.RTDB.getString(&fbDataObj, devicePath + "/groupId")) {
        String grp = fbDataObj.stringValue();
        if (grp.length() > 0 && grp != "null") {
          activeSession.groupCode = grp;
  ...
  // If userId is empty or node doesn't exist/permission error
  activeSession.isPaired = false;
  activeSession.uid = "";
  activeSession.displayName = "";
  activeSession.groupCode = "";
  currentMode = MODE_UNLINKED;
  Serial.print(F("[NODE SYNC Status] Unpaired / Fail reason: "));
